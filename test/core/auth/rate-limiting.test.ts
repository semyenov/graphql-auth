...
describe('Rate Limiting', () => {
    ...
    describe('Login rate limiting', () => {
        ...
        it('should allow login attempts within rate limit', async () => {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                if (response.body.kind === 'single' && response.body.singleResult.errors) {
                    ...
                    expect(response.body.kind === 'single' && response.body.singleResult.errors![0]!.message).toContain('Invalid email or password')
                }
            }
        })
        ...
        it('should block login attempts after exceeding rate limit', async () => {
            ...
            await gqlHelpers.expectGraphQLError<ResultOf<typeof LoginMutation>, VariablesOf<typeof LoginMutation>>(
                server,
                print(LoginMutation),
                variables,
                createMockContext(),
        ...
        it('should use email as identifier for rate limiting', async () => {
            ...
            expect(response.body.kind === 'single' && response.body.singleResult.errors![0]!.message).toContain('Invalid email or password')
            expect(response.body.kind === 'single' && response.body.singleResult.errors![0]!.message).not.toContain('Too many requests')
        })
    })
    ...
    describe('Signup rate limiting', () => {
        ...
        it('should block signup attempts after exceeding rate limit', async () => {
            ...
            for (let i = 0; i < 3; i++) {
                await executeOperation<ResultOf<typeof SignupMutation>, VariablesOf<typeof SignupMutation>>(
                    server,
                    print(SignupMutation),
                    {
                        email: baseEmail,
            ...
            await gqlHelpers.expectGraphQLError<ResultOf<typeof SignupMutation>, VariablesOf<typeof SignupMutation>>(
                server,
                print(SignupMutation),
                {
                    email: baseEmail,
