  ...
  return rule({ cache: 'strict' })(
    async (parent, args, context: Context) => {
      try {
        ...
        console.log(`Rate limit check for client ${clientId}: ${config.max}/${config.windowMs}ms`)
...
export const mutationRateLimitRule = createRateLimitRule(RATE_LIMIT_CONFIGS.mutation)
export const queryRateLimitRule = createRateLimitRule(RATE_LIMIT_CONFIGS.query)
export const strictRateLimitRule = createRateLimitRule(RATE_LIMIT_CONFIGS.strict)
 ...
 */
export function combineRateLimitRules(...rules: ReturnType<typeof createRateLimitRule>[]) {
